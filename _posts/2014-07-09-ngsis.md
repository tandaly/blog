...
> **操作前须知**
> 
>- 所有涉及Excel的文件必须是xls后缀且是2003版本文件，如你的电脑安装的是2007以上版本的office Excel，那么你就需要另存为`Excel 97-2003 工作薄`，格式为xls的文件。必须要通过客户端另存，不能直接修改后缀名，这样会无效的。
>


1. **配置EXCEL模板代码文件** 手动配置excel报表代码模块，并将模板文件(文件名必须和模板编码一致，如RP100.xls)放置代码模板文件目录`/upload/bm/view/change`下

2.  **配置数据库报表模板表** 在报表模板表`sis_report_model`中配置相关指标模板数据
	配置明细：
	- **GUID**：表主键，只要保证唯一即可
...
3. **部署测试** 启动服务器，在启动的期间，系统会自动根据模板表的配置自动生成报表
